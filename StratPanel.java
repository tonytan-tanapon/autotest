...
//		add( new TradesPanel() );
	...
	@Override public void realtimeBar(Bar bar) {
		...
		if (!m_req) {
			...
			ApiDemo.INSTANCE.controller().reqHistoricalData(m_contract, date, duration, queryLength.m_unit, barSize, WhatToShow.MIDPOINT, false, false, this);
			m_req = true;
			
			System.out.println(date+" "+ duration+" "+ queryLength.m_unit +""+barSize);
		}
		addBar( bar);
		m_chart.repaint();
	}
