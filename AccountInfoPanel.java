...
public class AccountInfoPanel extends JPanel implements INewTab, IAccountHandler {
	...
	public  MarginModel m_marginModel = new MarginModel();
	...
	public PositionsPanel m_positionsPanel  = new PositionsPanel();
...
//		tabbedPanel.addTab( "Market Value Summary", new MarketValueSummaryPanel() );
		tabbedPanel.addTab( "Positions (all accounts)",  m_positionsPanel);
...
//		});
	...
	/** Called when the tab is first visited. INewTab */
	...
	/** Called when the tab is closed by clicking the X. INewTab */
	...
	protected synchronized void onChanged() {
		...
		if (i != -1) {
			String selAcct = m_acctList.get( i);   // get account id, selAcct = "U651234"
			if (!selAcct.equals( m_selAcct) ) {   	// "U651234" != "", true
				m_selAcct = selAcct;		//m_selAcct= "U651234"
				m_marginModel.clear();
				m_mktValModel.clear();
...
//				System.out.println(tag+" "+value+" "+ currency);
				if(tag.equals("NetLiquidationByCurrency") && currency.equals("BASE")) {
					System.out.println("NetLiquidationByCurrency:"+ value);
